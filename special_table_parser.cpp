...
Special_table_parser::Special_table_parser(){
    this->rx.setPattern("[\\d\\S]");

}

void Special_table_parser::init(QString str)
{
    QString slr;
...
void Special_table_parser::run(QString str1)
{
    this->counter++;
    this->flag=0;
...
void Special_table_parser::adjuster(QString str, int c, int d){
    this->k++;
...
void Special_table_parser::runner(int v)
{
    if(v==0){
...
int Special_table_parser::tagcheck(){
    return 0;
}
